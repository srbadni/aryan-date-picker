...
export function DateRangePicker({ value, defaultValue = null, onChange, renderDay, minDate, maxDate, viewMode = 'infinite' }: DateRangePickerProps = {}) {
  ...
  return (
    <div style={{direction: adapter.direction}} className="adp-picker">
      ...
      <div className={`adp-range-calendars-mobile${viewMode === 'single' ? ' adp-range-calendars-mobile-single' : ''}`}>
        {viewMode === 'single' ? (
          <button
            type="button"
            className="adp-nav-button adp-range-nav-button"
            onClick={() => canNavigateMobilePrev && setConstrainedVisibleMonthStart(adapter.addMonths(constrainedVisibleMonthStart, -1), mobileMonthCount)}
            disabled={!canNavigateMobilePrev}
            aria-label="Previous month"
          >
            ‹
          </button>
        ...
        {viewMode === 'single' ? (
          <button
            type="button"
            className="adp-nav-button adp-range-nav-button"
            onClick={() => canNavigateMobileNext && setConstrainedVisibleMonthStart(adapter.addMonths(constrainedVisibleMonthStart, 1), mobileMonthCount)}
            disabled={!canNavigateMobileNext}
            aria-label="Next month"
          >
            ›
          </button>
